...
/**
 * Classes to enforce the various access rules that can apply to a hippotrack.
 *
 ...
 * hippotrack, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
...
class hippotrack_access_manager {
    /** @var hippotrack the hippotrack settings object. */
    ...
    /**
     * Create an instance for a particular hippotrack.
     * @param object $hippotrackobj An instance of the class hippotrack from attemptlib.php.
     *      The hippotrack we will be controlling access to.
     ...
     */
    public function __construct($hippotrackobj, $timenow, $canignoretimelimits) {
        $this->hippotrackobj = $hippotrackobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($hippotrackobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular hippotrack.
     ...
     */
    protected function make_rules($hippotrackobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($hippotrackobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['hippotrackaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('hippotrackaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the hippotrack table.
     *
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($hippotrackform, $mform);
        }
    }
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $hippotrackform);
        }
    ...
    /**
     * Save any submitted settings when the hippotrack settings form is submitted.
     *
    ...
    public static function save_settings($hippotrack) {
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($hippotrack);
        }
    }

    /**
     * Delete any rule-specific settings when the hippotrack is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the hippotrack table.
     *
    ...
    public static function delete_settings($hippotrack) {
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($hippotrack);
        }
    }
     ...
     */
    protected static function get_load_sql($hippotrackid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{hippotrack} hippotrack';
        $allparams = array('hippotrackid' => $hippotrackid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($hippotrackid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE hippotrack.id = :hippotrackid", $allparams);
    }
     ...
     */
    public static function load_settings($hippotrackid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($hippotrackid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($hippotrackid);
        }
    ...
    /**
     * Load the hippotrack settings and any settings required by the access rules.
     ...
     *
     * @param int $hippotrackid the hippotrack id.
     * @return object mdl_hippotrack row with extra fields.
     */
    public static function load_hippotrack_and_settings($hippotrackid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($hippotrackid, $rules, 'hippotrack.*');
        $hippotrack = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($hippotrackid) as $name => $value) {
                $hippotrack->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this hippotrack, such
     * as is shown at the top of the hippotrack view page. Note that not all
    ...
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        return new mod_hippotrack_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'hippotrackobj' => $this->hippotrackobj,
     ...
     * hippotrack. Used, for example, to change the label by the grade displayed on the view page from
    ...
    /**
     * @return bolean if this hippotrack should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the hippotrack view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_hippotrack_renderer $output the hippotrack renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->hippotrackobj->view_url(), $message);
            die();
        } else {
            redirect($this->hippotrackobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the hippotrack, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        $when = hippotrack_attempt_state($this->hippotrackobj->get_hippotrack(), $attempt);
        $reviewoptions = mod_hippotrack_display_options::make_from_hippotrack(
                $this->hippotrackobj->get_hippotrack(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->hippotrackobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->hippotrackobj->review_url($attempt->id),
