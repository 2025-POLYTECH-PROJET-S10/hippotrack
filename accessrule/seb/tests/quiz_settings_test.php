...
<plist version=\"1.0\"><dict><key>showTaskBar</key><true/><key>allowWlan</key><false/><key>showReloadButton</key><true/>"
    ...
    public function test_browser_exam_keys_are_retrieved_as_array() {
        $quizsettings = new hippotrack_settings();
     ...
     *
     * @param hippotrack_settings $quizsettings Given quiz settings instance.
    ...
    /**
     * Test that we can get hippotrack_settings by quiz id.
     */
    public function test_get_hippotrack_settings_by_hippotrack_id() {
        $expected = hippotrack_settings::get_record(['quizid' => $this->quiz->id]);

        $this->assertEquals($expected->to_record(), hippotrack_settings::get_by_hippotrack_id($this->quiz->id)->to_record());

        // Check that data is getting from cache.
        $expected->set('showsebtaskbar', 0);
        $this->assertNotEquals($expected->to_record(), hippotrack_settings::get_by_hippotrack_id($this->quiz->id)->to_record());
    ...
    public function test_get_config_by_hippotrack_id() {
        ...
        $this->assertEquals($expected, hippotrack_settings::get_config_by_hippotrack_id($this->quiz->id));

        // Check that data is getting from cache.
        $quizsettings->set('showsebtaskbar', 0);
        $this->assertNotEquals($quizsettings->get_config(), hippotrack_settings::get_config_by_hippotrack_id($this->quiz->id));

        // Now save and check that cached as been updated.
        $quizsettings->save();
        $this->assertEquals($quizsettings->get_config(), hippotrack_settings::get_config_by_hippotrack_id($this->quiz->id));
    ...
    public function test_get_config_key_by_hippotrack_id() {
        ...
        $this->assertEquals($expected, hippotrack_settings::get_config_key_by_hippotrack_id($this->quiz->id));

        // Check that data is getting from cache.
        $quizsettings->set('showsebtaskbar', 0);
        $this->assertNotEquals($quizsettings->get_config_key(), hippotrack_settings::get_config_key_by_hippotrack_id($this->quiz->id));

        // Now save and check that cached as been updated.
        $quizsettings->save();
        $this->assertEquals($quizsettings->get_config_key(), hippotrack_settings::get_config_key_by_hippotrack_id($this->quiz->id));
