...
M.mod_hippotrack.timer = {
     ...
     */
    stop: function (e) {
        if (M.mod_hippotrack.timer.timeoutid) {
            clearTimeout(M.mod_hippotrack.timer.timeoutid);
        }
    },
    ...
    update: function () {
        ...
        if (secondsleft < 0) {
            ...
            M.mod_hippotrack.timer.FormChangeChecker.markFormSubmitted(input.getDOMNode());
            form.submit();
            return;
        }
    ...
    updateEndTime: function (timeleft) {
        ...
        if (Math.abs(newtimeleft - M.mod_hippotrack.timer.endtime) > M.mod_hippotrack.timer.threshold) {
            M.mod_hippotrack.timer.endtime = newtimeleft;
            M.mod_hippotrack.timer.update();
        }
    }
};

M.mod_hippotrack.filesUpload = {
    /**
     * YUI object.
     */
    Y: null,
...
M.mod_hippotrack.nav = M.mod_hippotrack.nav || {};
...
M.mod_hippotrack.nav.init = function (Y) {
    ...
    require(['core_form/events'], function (formEvent) {
        document.addEventListener(formEvent.eventTypes.uploadStarted, function () {
            M.mod_hippotrack.filesUpload.numberFilesUploading++;
            M.mod_hippotrack.filesUpload.disableNavPanel();
        });
    ...
    if (M.core_question_flags) {
        M.core_question_flags.add_listener(M.mod_hippotrack.nav.update_flag_state);
    }
};

M.mod_hippotrack.secure_window = {
    init: function (Y) {
        if (window.location.href.substring(0, 4) == 'file') {
            window.location = 'about:blank';
        }
        Y.delegate('contextmenu', M.mod_hippotrack.secure_window.prevent, document, '*');
        Y.delegate('mousedown', M.mod_hippotrack.secure_window.prevent_mouse, 'body', '*');
        Y.delegate('mouseup', M.mod_hippotrack.secure_window.prevent_mouse, 'body', '*');
        Y.delegate('dragstart', M.mod_hippotrack.secure_window.prevent, document, '*');
        Y.delegate('selectstart', M.mod_hippotrack.secure_window.prevent_selection, document, '*');
        Y.delegate('cut', M.mod_hippotrack.secure_window.prevent, document, '*');
        Y.delegate('copy', M.mod_hippotrack.secure_window.prevent, document, '*');
        Y.delegate('paste', M.mod_hippotrack.secure_window.prevent, document, '*');
        ...
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'press:67,86,88+ctrl');
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'up:67,86,88+ctrl');
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'down:67,86,88+ctrl');
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'press:67,86,88+meta');
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'up:67,86,88+meta');
        Y.on('key', M.mod_hippotrack.secure_window.prevent, '*', 'down:67,86,88+meta');
    },
    ...
    is_content_editable: function (n) {
        ...
        return M.mod_hippotrack.secure_window.is_content_editable(n);
    },
    ...
    prevent_mouse: function (e) {
        ...
        if (e.button == 1 && M.mod_hippotrack.secure_window.is_content_editable(e.target)) {
    ...
    init_close_button: function (Y, url) {
        Y.on('click', function (e) {
            M.mod_hippotrack.secure_window.close(url, 0)
        }, '#secureclosebutton');
    },
