...
/**
 * Class to calculate and also manage caching of hippotrack statistics.
 *
 * These hippotrack statistics calculations are described here :
 *
 * http://docs.moodle.org/dev/HippoTrack_statistics_calculations#Test_statistics
 *
...
class calculator {
    ...
    /**
     * Compute the hippotrack statistics.
     *
     ...
     * @return calculated $hippotrackstats The statistics for overall attempt scores.
     */
    public function calculate($hippotrackid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $hippotrackstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($hippotrackid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $hippotrackstats->{$propertyname} = $value;
        }
        ...
        if ($s != 0) {
            ...
            list($fromqa, $whereqa, $qaparams) =
                hippotrack_statistics_attempts_sql($hippotrackid, $groupstudentsjoins, $whichattempts);

            $hippotrackstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($hippotrackstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $hippotrackstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/HippoTrack_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $hippotrackstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $hippotrackstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $hippotrackstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $hippotrackstats->errorratio = 100 * sqrt(1 - ($hippotrackstats->cic / 100));
                            $hippotrackstats->standarderror = $hippotrackstats->errorratio *
                                $hippotrackstats->standarddeviation / 100;
                        }
                    }
            ...
            $hippotrackstats->cache(hippotrack_statistics_qubaids_condition($hippotrackid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $hippotrackstats;
    }
    ...
    /**
     * Given a particular hippotrack grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    /**
     * Given a particular hippotrack grading method return a string id for use as a field name prefix in mdl_hippotrack_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/HippoTrack_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($hippotrackid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(hippotrack_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = hippotrack_statistics_attempts_sql($hippotrackid, $groupstudentsjoins, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/HippoTrack_statistics_calculations#Median_Score
     *
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT hippotracka.id, hippotracka.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/HippoTrack_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((hippotracka.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((hippotracka.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((hippotracka.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
