@mod @mod_hippotrack
Feature: Allow students to redo questions in a practice hippotrack, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a hippotrack as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | hippotrack     | HippoTrack 1 | HippoTrack 1 description | C1     | hippotrack1    | immediatefeedback  | 1                |
  ...
  Scenario: After redoing a question, regrade works
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I press "Try another question like this one"
    And I am on the "HippoTrack 1" "mod_hippotrack > Grades report" page logged in as "teacher"
  ...
  Scenario: The redo question button is visible but disabled for teachers
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I log out
    And I am on the "HippoTrack 1" "mod_hippotrack > View" page logged in as "teacher"
  ...
  Scenario: Teachers reviewing can see all the questions attempted in a slot
    ...
    And I am on the "HippoTrack 1" "mod_hippotrack > Statistics report" page logged in as teacher
    And I follow "TF1"
    And "False" row "Frequency" column of "hippotrackresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "hippotrackresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "hippotrackresponseanalysis" table should contain "100.00%"
  ...
  Scenario: Teachers reviewing can switch between attempts in the review question popup
    ...
    # Now switch to the other hippotrack attempt using the link at the top, which does not have a redo.
  ...
  Scenario: Redoing questions should work with random questions as well
    Given the following "activities" exist:
      | activity | name   | intro              | course | idnumber | preferredbehaviour | canredoquestions |
      | hippotrack     | HippoTrack 2 | HippoTrack 2 description | C1     | hippotrack2    | immediatefeedback  | 1                |
    And I am on the "HippoTrack 2" "mod_hippotrack > Edit" page logged in as "admin"
    And I open the "last" add to hippotrack menu
    And I follow "a random question"
    And I press "Add random question"
    And user "student" has started an attempt at hippotrack "HippoTrack 2" randomised as follows:
      | slot | actualquestion |
      | 1    | TF1            |
    And I am on the "HippoTrack 2" "mod_hippotrack > View" page logged in as "student"
  ...
  Scenario: Teachers reviewing can see author of action in review attempt
    ...
    And the following "activities" exist:
      ...
      | hippotrack     | HippoTrack 2 | HippoTrack 2 description | C1     | hippotrack2    |
    And hippotrack "HippoTrack 2" contains the following questions:
      | question | page |
      | SA1      | 1    |
    And user "student" has attempted "HippoTrack 2" with responses:
      | slot | response    |
      | 1    | J.K.Rowling |
    And I am on the "HippoTrack 2" "mod_hippotrack > Manual grading report" page logged in as "teacher"
